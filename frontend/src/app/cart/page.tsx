              ...
              {items.map((item) => (
                <div key={item.productId} className="card flex items-center gap-4">
                  ...
                  <div className="flex items-center rounded-xl border border-black/10">
                    <button className="px-3 py-1.5" onClick={() => updateQty(item.productId, item.qty - 1)}>−</button>
                    <span className="w-8 text-center text-sm">{item.qty}</span>
                    <button className="px-3 py-1.5" onClick={() => updateQty(item.productId, item.qty + 1)}>+</button>
                  </div>
                  <button onClick={() => removeItem(item.productId)} className="text-ink/40 hover:text-red-600">
                    <Trash2 size={18} />
                  </button>
                </div>
              ))}
